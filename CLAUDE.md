...
- **Phase 4 in progress:** 4.1–4.6 done — the Aurora renderer exists and renders the full page from `EventContent` alone (`src/templates/aurora/`), reachable at `/templates/[templateId]/preview` (FR-50's Live Preview, pulled forward so every later task has a visual target). 198 unit tests. 4.7 (final demo copy + imagery + fallback verification), 4.8 (responsive/a11y/Lighthouse), and 4.9 (public homepage grid) remain.
