...
- [x] 10.1 (M) E2E smoke suite (Playwright): homepage → Live Preview; homepage → Edit → login → create → edit → submit → approve → public view; report flow; preview link flow.

  **Outcome.** `@playwright/test` added; `playwright.config.ts` + `e2e/` (setup, support, four specs). Five flows across seven tests, all green against the dev server: homepage + Live Preview (`public.spec.ts`), Edit → login → create handoff (`auth-handoff.spec.ts`), the full lifecycle create → edit(autosave) → submit → admin approve → public view → **report on the live site** (`lifecycle.spec.ts`, folding the report flow onto the site it publishes), and the preview-link share → view → revoke → dead-end (`preview-link.spec.ts`). Design decisions worth keeping: a `setup` project seeds two verified accounts and saves each one's signed-in **storage state**, which the tests load instead of logging in — sign-ins are otherwise rate-limited to 5/min (task 9.4), and the one test that must exercise the login form (`auth-handoff`) runs signed-out and is the only real UI sign-in. Verification can't go through the browser (Better Auth won't session an unverified email and dev sends no mail), so `e2e/support/db.ts` flips `emailVerified`/`role` with **raw `pg`** — not the generated Prisma client, which is ESM and un-`require`-able from Playwright's CJS transform. Accounts and the signup hash come from Better Auth's own `/api/auth/sign-up/email` (with an explicit `Origin`, or it 403s `MISSING_OR_NULL_ORIGIN`). Events use unique `eee`-prefixed lowercase slugs; global teardown deletes them by prefix (cascades clean the rest). Must run on **port 3000** — Better Auth signs cookies against `NEXT_PUBLIC_APP_URL` and rejects other origins. CI integration pending Mohammad's approval (touches `.github/workflows/ci.yml`).

- [x] 10.2 (S) Security pass: authorization audit on every action/handler, header hardening (CSP for public sites), URL/output sanitization review.

  **Outcome.** _Authorization audit_ — all 44 Server Actions across the three `actions.ts` files authenticate (`getAuthenticatedUser`/`getAdminUser`) and delegate to a service that re-authorizes: editor/event mutations through `loadManageable` → `canManageEvent` (owner-or-admin), admin actions through the admin re-check in each service (defence-in-depth over the layout guard). The only anonymous handler is `POST /api/reports` (deliberate — honeypot + IP-hash rate limit); `/api/auth` is Better Auth's own; `/api/health` is a read-only `SELECT 1`. No code change needed — the pattern already holds everywhere. _Headers/CSP_ — `next.config.ts` now applies a global CSP plus `X-Content-Type-Options`, `X-Frame-Options: DENY`, `Referrer-Policy`, `Permissions-Policy`, and production-only HSTS. The CSP is **nonce-free by necessity**: a nonce forces every page dynamic, which would kill the static/PPR rendering the public sites depend on (confirmed against the Next.js CSP guide via Context7). `script-src 'self' 'unsafe-inline'` is safe here because the injection surface is closed — **no `dangerouslySetInnerHTML` anywhere** (audited), React escapes all organizer text, and user URLs render only through validated `http(s)` attributes. Dev-only allowances (`'unsafe-eval'`, HMR `ws:`) and prod-only `upgrade-insecure-requests` are branched on `NODE_ENV`. Aurora is CSP-clean: its reveals/parallax are external CSS-class scroll animations (the old inline `opacity:0` was removed in Phase 4), and `style-src 'unsafe-inline'` covers the editor's `style` attributes. _URL/output_ — every user URL (`socialLinks.url`, sponsor `websiteUrl`, `registrationUrl`, TED URL) is `externalUrlSchema` (protocol `^https?$`), reused across `event-content.ts`/`content.ts`/`event.ts`; templates render external links with `rel="noopener noreferrer"`; snapshots are re-validated on read through the upgrader, so a stored `javascript:` URL can't reach a template. All 7 E2E flows pass **under the CSP** on the dev server (hydration, every form, the report `fetch`, public rendering); the production policy (no `unsafe-eval`) is the Next.js-documented one and is re-confirmed in the browser by 10.3's production Lighthouse pass.

- [x] 10.3 (S) Accessibility + performance final audit (app and public template).
  ...
  **Outcome.** Took the second option (the fuller fix). Geist is removed from the root layout and declared once in `src/app/fonts.ts`, applied by the four app-chrome group layouts (`(marketing)`, `(auth)`, `(app)`, `admin`) — so it is no longer on the `<html>` the public `[site]` route inherits. Aurora was given its **own** mono (`--font-aurora-mono`, in `templates/aurora/fonts.ts`), and the countdown now uses it instead of Tailwind's `font-mono` → the event site no longer depends on any app font. `globals.css` gained system-`ui` fallbacks on `--font-sans`/`--font-mono` so the root-level error/404 pages (which render under the root layout with no group, and so no Geist var) degrade to system fonts rather than the browser serif. **Measured on a production build**: the CSS served to `/templates/aurora/preview` (the event-site rendering path) declares **Archivo + Inter + Geist Mono and no Geist Sans**, while `/login` (app chrome) declares **Geist Sans + Geist Mono** — Geist Sans is off the event-site critical path, confirmed. (Note: Next 16 + Turbopack ships `next/font` via CSS `@font-face`, not `<link rel=preload>`, so the win is the removed @font-face + subset files, not a removed preload tag.) Accessibility structure re-checked on the built output: `<html lang>`, single `<h1>`, full landmark set (header/nav/main/section/footer), and every `<img>` carries `alt` (decorative ones `alt=""`) — no regression from the a11y-100 baselines of Phases 4/8. All 7 E2E flows pass; `next/font` mock extended (`Geist`, `Geist_Mono`) so the 376 unit tests still pass. **The numeric Lighthouse run against the deployed live site (target 94 / 100 / 100 / 100, LCP 1.6s) is folded into the 10.6 launch checklist** — it needs the redeployed site, and this change only reduces the event-site payload, so it cannot regress the Phase 8 baseline.

- [x] 10.4 (S) Operational polish: error monitoring hook points, structured logging in services, orphaned-media cleanup script.

  **Outcome.** `src/server/logger.ts` — a dependency-free structured logger (one JSON line per event, level-routed) plus `captureException`, the **single error-monitoring seam** where an APM/Sentry gets wired (one edit, nowhere else); the ad-hoc `console.*` sites in `publish-notifications.tsx` (a fire-and-forget email failing is exactly what it exists to surface) and `report-service.ts` now go through it. The **orphaned-media cleanup** is `media-cleanup-service.ts` + `scripts/cleanup-orphaned-media.ts` (dry-run by default, `--apply` to delete): an asset is reaped only when it has no draft FK (repository query, past a `ORPHANED_MEDIA_GRACE_HOURS` window) **and** no retained snapshot references its public id — the second half is `collectImagePublicIds` (`content/image-refs.ts`, pure, 3 tests) run over `listAllSnapshotContents`, which throws on an unparseable snapshot so the sweep aborts rather than risk deleting a live image. Verified against the local DB: dry run found 1 unreferenced orphan and left it. The periodic **rate-limit sweep** is `GET /api/cron/sweep` (calls the existing `sweepExpiredRateLimits`), fail-closed on `CRON_SECRET` (`Authorization: Bearer …`; refuses everyone in production when the secret is unset, open only in dev — verified returning `{ok:true,swept:1}` locally). 379 unit tests. **Deferred (Mohammad's call): the `vercel.json` cron schedule was NOT added** — the endpoint works when called, but nothing invokes it on a schedule yet. Launch step (10.6): create `vercel.json` with `crons: [{ path: "/api/cron/sweep", schedule: "0 3 * * *" }]` and set `CRON_SECRET` in the Vercel dashboard (Hobby plan caps crons at once/day).

- [x] 10.5 (S) Homepage polish: finalize value-proposition copy and SEO metadata around the template grid built in 4.9; verify the Live Preview and Edit → login → create-event handoff end-to-end (do not rebuild — the homepage itself ships in Phase 4).

  **Outcome.** No rebuild — the Phase 4 homepage and grid stand. Two real fixes. _Copy_: the subhead advertised "schedule", which V1 deliberately has no section for (out of scope) — changed to "speakers, venue, sponsors, and event details" so the promise matches what an organizer can actually fill in. _SEO_: the page set `title` as a plain string, which the root layout's `%s · Tedxplore` template would have double-branded ("Tedxplore — … · Tedxplore"); switched to `title.absolute`, and added `canonical: "/"` + Open Graph/Twitter (`summary` card — no honest single image for a multi-template platform), all resolving against the root `metadataBase`. The headline is one shared const so the tab title and social cards can't drift. Confirmed over HTTP: `<title>Tedxplore — premium TEDx event websites from your content</title>` (single brand), canonical + `og:title` present. Both required flows re-verified green by the existing E2E specs (`public.spec.ts` homepage + Live Preview, `auth-handoff.spec.ts` Edit → login → create).

- [~] 10.6 (S) Production env setup, domain, seed admin, launch checklist run.

  **Outcome (code/doc ready; the launch itself is Mohammad's to run).** `LAUNCH.md` is the go-live runbook — an ordered, owner-marked checklist consolidating the standard deploy steps with Phase 10's new/deferred items. Already true in production: Vercel + Neon + Resend + Cloudinary configured, and `ma.shamshiri@gmail.com` is `ADMIN` on the prod DB (`scripts/grant-admin.ts`; no admin-granting UI by design). **Remaining, and all Mohammad's (dashboard/deploy/DNS/admin-login — not things I can or should do):** set `CRON_SECRET` in Vercel; push `main` to deploy Phase 10 (with CI green, incl. the new `e2e` job); add the `vercel.json` cron (deferred from 10.4); run Lighthouse on a live `/tedx{slug}` to confirm the 10.3 target numbers; the live-site browser smoke and header check; and — when `tedxplore.com` is acquired (not owned yet) — the domain + `NEXT_PUBLIC_APP_URL` + Resend SPF/DKIM (which also fixes the `onboarding@resend.dev` deliverability caveat). Marked `[~]` (prepared, not run) rather than `[x]`: the production launch is a human action against live infrastructure.

**Exit:** production launch — **runbook ready in `LAUNCH.md`; execution pending Mohammad.**
