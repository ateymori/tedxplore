...
- [ ] 4.1 (M) Template shell: the registry entry already exists (3.0) — add `Renderer` to `TemplateDefinition` and build it. Layout, typography scale, color system, motion primitives (scroll reveal, parallax restraint, `prefers-reduced-motion`), smooth scrolling + section nav.
