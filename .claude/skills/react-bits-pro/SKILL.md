...
> ## ⚠️ Tedxplore project overrides (these win over the generic guidance below)
>
> This is the Tedxplore repo. Its React Bits setup is already done and differs
> from the vendor defaults below — **`CLAUDE.md` (§ "React Bits") is the source
> of truth**; where this file disagrees, follow CLAUDE.md:
>
> - **Install with `pnpm dlx`, never `npx`.** The local shadcn CLI is broken by
>   this repo's zod-4 override; `pnpm dlx shadcn@latest add …` resolves its own
>   zod and works.
> - **The license key is `REACTBITS_LICENSE_KEY` in `.env`** (already set,
>   gitignored), not `.env.local`. `components.json` already carries the three
>   registries with `Authorization: Bearer ${REACTBITS_LICENSE_KEY}`. Never paste
>   the real key anywhere.
> - **Registries are already merged** into `components.json` — do **not** re-run
>   `init` or re-add them.
> - **No install-folder convention.** Lint is relaxed project-wide (the strict
>   React Compiler-era `react-hooks` rules are off), so a React Bits component
>   lints clean **anywhere** — app chrome *or* inside a template
>   (`src/templates/*`). Install where it belongs; `--path <dir>` is optional.
> - **Free-registry component ids are PascalCase + variant suffix** for this
>   TS+Tailwind repo, e.g. `SplitText-TS-TW` (not `split-text`).
> - After installing: check the export shape, install named peer deps, and
>   re-run `pnpm typecheck` / `pnpm lint`. Inside a template, remember React Bits
>   components are client-side JS animation — fine below the fold, mind LCP for
>   hero content.
...
This document is the single source of truth for the vendor's mechanics (naming,
imports, WebGL sizing); for anything project-specific the overrides above win.
...
| `@reactbits-pro` | 238 page-section blocks (21 categories) | **Pro** | `@reactbits-pro/<slug>` |
...
## Appendix B — Block catalog (`@reactbits-pro`, 238, Pro tier)
